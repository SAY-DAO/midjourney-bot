...
log_format = logging.Formatter(
    "[%(levelname)s] [%(pathname)s:%(lineno)d] - %(message)s - [%(process)d:%(thread)d]"
)

file_handler = logging.FileHandler(
    "midjourney_automation.log", mode="a"
)  # Create file handler
...
@eel.expose
async def download_upscaled_images(need, page, prompt_text: str):
    try:
        ...
        if "Vary (Strong)" in message_text and "Web" in message_text:
            try:
                ...
                for image in last_four_images:
                    ...
                    download_response = requests.get(url, stream=True, timeout=500000)
                    i += 1
                    fileName = f'need-images/{"need"+"-"+str(need["flaskId"])}/{str(need["flaskId"]) +"_"+str(i)}.png'
            ...
            await download_upscaled_images(need, page, prompt_text)
...
@eel.expose
async def wait_and_select_upscale_options(need, page, prompt_text: str):
    """
    ...
    try:
        ...
        while i == 0:
            ...
            if "U1" in last_message:
                ...
                await download_upscaled_images(need, page, prompt_text)
                i = 1  # Exit the loop when upscale options have been found and selected
...
@eel.expose
async def generate_prompt_from_json(page, need):
    try:
        prompt_text = need["needRetailerImg"] + ", " + need["prompt"]
        random_sleep()
        ...
        await wait_and_select_upscale_options(need, page, prompt_text)
...
@eel.expose
async def send_bot_command(page, need, command: str):
    try:
        logger.info("Clicking on chat section ...")
        chat_section = page.locator(
        ...
        await chat_section.fill(command)
        await asyncio.sleep(random.randint(1, 5))
        ...
        logger.info("Generating prompt using nest.saydao.org jon file.")
        await generate_prompt_from_json(page, need)
...
@eel.expose
async def open_discord_channel(page, need, channel_url: str, bot_command: str):
    try:
        ...
        await send_bot_command(page, need, bot_command)
                ...
                while True:
                    try:
                        ...
                        if (
                            ...
                            logger.info(
                                f"Skipping need: {need['flaskId']} since it exists!"
                            )
                    except Exception as e:
                        logger.error(
                            f"Error occurred: {e} while executing the main function loop."
                        )
                    break
...
def start_bot():
    bot_command = "/imagine"
    channel_url = "https://discord.com/channels/1130406262786490480/1130406263231098902"
    try:
        logger.info(f"Starting...")

        asyncio.run(main(bot_command, channel_url))
